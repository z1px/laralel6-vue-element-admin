<template>
    <div class="app-container">
        <div class="filter-container">
            ...
            <!--<router-link v-if="checkPermission('admin.permissions.add')" :to="{path: '/manage/permissions/add'}">
                <el-button
                    v-if="checkPermission('admin.permissions.add')"
                    type="primary"
                    class="filter-item"
                    icon="el-icon-edit"
        ...
        <el-table v-loading="loading" :data="data" fit highlight-current-row style="width: 100%">
            ...
            <el-table-column prop="title" label="权限名称" width="120" />
            <el-table-column prop="route_name" label="路由名" width="120" />
            <el-table-column prop="brief" label="权限简介" width="320" />
            <el-table-column prop="pname" label="上级权限" width="120" />
            <el-table-column label="状态" width="180" align="center">
                <template slot-scope="scope">
                    <el-switch
                        v-if="checkPermission('admin.permissions.update')"
            ...
            <el-table-column label="操作" min-width="150" align="left" fixed="right">
                <template slot-scope="scope">
                    <!--<router-link v-if="checkPermission('admin.permissions.update')" :to="{path: '/manage/permissions/update/', query: {id: scope.row.id}}">
                    ...
                    <el-popconfirm
                        v-if="checkPermission('admin.permissions.delete')"
        ...
        <el-dialog
            ...
            <el-form
                ...
                <el-form-item label="上级权限" prop="pid">
                    <el-select v-model="dialogFormData.pid" clearable filterable placeholder="请选择上级权限">
                        <el-option
                            v-for="item in list_permission"
                            :key="item.id"
                            :value="item.id"
                            :label="item.title"
                            :disabled="item.id === dialogFormData.id">
                            <span v-if="item.pname">{{ item.pname }} => </span>{{ item.title }}
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="权限名称" prop="title">
                    <el-input v-model="dialogFormData.title" placeholder="权限名称" />
                </el-form-item>
                <el-form-item label="路由名" prop="route_name">
                    <el-input v-model="dialogFormData.route_name" placeholder="请输入路由名" />
                </el-form-item>
...
<script>
    import {getList, getAll, add, update, del} from '@/api/permissions';
    ...
    export default {
        ...
        methods: {
                ...
                getAll().then(response => {
                    ...
                    if(response.data.length > 0){
                        response.data.map((data) => {
                            if(data.pid > 0){
                                let index = this.list_permission.findIndex((value) => {
                                    return data.pid === value.id;
                                });
                                this.list_permission.splice(index + 1, 0, data);
                            }else{
                                this.list_permission.push(data);
                            }
                        });
                    }
            ...
            add() {
                this.isUpdate = false;
                this.dialogFormTitle = this.$t('route.permissions.add');
            ...
            update(data) {
                this.isUpdate = true;
                this.dialogFormTitle = this.$t('route.permissions.update');
                this.dialogFormVisible = true;
